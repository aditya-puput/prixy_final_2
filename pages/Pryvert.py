...
logo_path = f'{images_directory}/{logo_filename}'
...
with col1:               # To display the header text using css style
    ...
    st.markdown('<p class="font">Welcome to Pry Convert Images</p>', unsafe_allow_html=True)
...
uploaded_file = st.file_uploader("Upload Photo", type=['jpg', 'png', 'jpeg'], key="uploaded_file")
...
with st.sidebar.form(key='columns_in_form', clear_on_submit=True):
    rating_label = 'Please rate the app'
    rating = st.slider(rating_label, min_value=1, max_value=5, value=3, help='Drag the slider to rate the app. This is a 1-5 rating scale where 5 is the highest rating')

    feedback_label = 'Please leave your feedback here'
    text = st.text_input(label=feedback_label)
